				...
				case "chart1":
					...
					for (x = 0; x < chartvalues1.length; x++) {
						...
						$.ajax({
							...
							url:"inner_resultado_3_general.php",
							type:"POST",
							data: values,
							dataType:"xml",
							success: function() {
									...
									success: function(xml) {
										...
										if (returnValues_1 == 1) {
											if (stopPropagation) {
												...
												statusBox('loading','NULL','NULL','remove','NULL');
												window.location.href = "inner_resultado_2_general.php?id="+tempId+"&reportidoriginal="+$("#reportid1").val()+"&reportidupdated="+$("#reportid2").val()+"&labid="+$("#labid").val();
											}
										}
									}
								});															
