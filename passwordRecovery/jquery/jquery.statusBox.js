...
function statusBox(val,val2,val3,val4,val5) {
	...
	//check for jQuery
	if(!window.jQuery) {
		alert('jQuery no parece estár instalado correctamente, por favor verifique su instalación de jQuery antes de uesar statusBox.');
		throw "jQuery has not been found.";
	}
	
	//check for Bootstrap
	if (!typeof $().modal == 'function') {
		alert('Bootstrap no parece estár instalado correctamente, por favor verifique su instalación de Bootstrap antes de uesar statusBox.');
	...
	if (document.getElementById('statusBox_'+val.toLowerCase()+'Box')) {
		...
		switch(val.toLowerCase()) {
			...
			case'info':
				...
				//verify if there's any custom text to introduce into the box's text.
				if (val3.toLowerCase() == 'null') {
					innerDivContentText = "Se ha encontrado una excepción, por favor comuniquese con el administrador de la aplicación.";
				} else {
					innerDivContentText = val3;
				}
