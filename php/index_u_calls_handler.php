...
switch ($header) {
	...
	case 'showMethod_analito':
		$filterArray = explode("|", $filter);
		// JS envía: analizador|analito
		$analizadorId = (isset($filterArray[0]) && $filterArray[0] != "") ? (int) $filterArray[0] : 0;
		$analitoId = (isset($filterArray[1]) && $filterArray[1] != "") ? (int) $filterArray[1] : 0;

		$checkExistsQry = "SELECT COUNT(*) as total FROM analito_metodologia_unidad WHERE id_analito = $analitoId";
		$checkRes = mysql_query($checkExistsQry);
		$checkData = mysql_fetch_assoc($checkRes);

		if ($checkData['total'] > 0) {
			$qry = "SELECT m.id_metodologia, m.nombre_metodologia 
                    FROM $tbl_metodologia m
                    INNER JOIN analito_metodologia_unidad amu ON m.id_metodologia = amu.id_metodologia
                    WHERE amu.id_analito = $analitoId
                    GROUP BY m.id_metodologia
                    ORDER BY m.nombre_metodologia ASC";
			...
			while ($qryData = mysql_fetch_array($qryArray)) {
				$rv1[$x] = "";
				$rv2[$x] = "";
				$rv3[$x] = $qryData['id_metodologia'];
				$rv4[$x] = $qryData['nombre_metodologia'];
				$x++;
			}

			echo '<response code="1">';
			echo '<returnvalues1 selectomit="1">' . implode("|", $rv1) . '</returnvalues1>';
			echo '<returnvalues2 selectomit="1">' . implode("|", $rv2) . '</returnvalues2>';
			echo '<returnvalues3 content="id">' . implode("|", $rv3) . '</returnvalues3>';
			echo '<returnvalues4>' . implode("|", $rv4) . '</returnvalues4>';
			echo '</response>';
			...
			switch ($filterid) {
				case 'id_analito':
					$filterQry = "WHERE $tbl_analizador.id_analizador = " . $analizadorId;
					break;
				case 'id_analizador':
	...
	case 'showReactive':
		...
		echo '</response>';
		break;
	case 'showUnit_analito':
		$filterArray = explode("|", $filter);
		$id_analito = (isset($filterArray[0]) && $filterArray[0] != "") ? (int) $filterArray[0] : 0;
		$id_metodologia = (isset($filterArray[1]) && $filterArray[1] != "") ? (int) $filterArray[1] : 0;
		$id_analizador = (isset($filterArray[2]) && $filterArray[2] != "") ? (int) $filterArray[2] : 0;

		$checkQry = "SELECT COUNT(*) as total FROM analito_metodologia_unidad 
                 WHERE id_analito = $id_analito AND id_metodologia = $id_metodologia";
		$checkRes = mysql_query($checkQry);
		$checkData = mysql_fetch_assoc($checkRes);
		...
		if ($checkData['total'] > 0) {
			$qry = "SELECT u.id_unidad, u.nombre_unidad 
                FROM $tbl_unidad u
                INNER JOIN analito_metodologia_unidad amu ON u.id_unidad = amu.id_unidad
                WHERE amu.id_analito = $id_analito AND amu.id_metodologia = $id_metodologia
			...
			while ($qryData = mysql_fetch_array($qryArray)) {
				$rv1[$x] = "";
				$rv2[$x] = "";
				$rv3[$x] = $qryData['id_unidad'];
				$rv4[$x] = $qryData['nombre_unidad'];
				$x++;
			}
		} else {
			$filterQry = "WHERE $tbl_unidad.id_unidad IN (
                        SELECT id_unidad FROM $tbl_configuracion_laboratorio_analito 
                        WHERE id_analizador = $id_analizador AND id_analito = $id_analito
                    )";

			$qry = "SELECT $tbl_unidad_analizador.id_conexion, $tbl_unidad.id_unidad, $tbl_unidad.nombre_unidad, $tbl_analizador.nombre_analizador 
                FROM $tbl_unidad 
                INNER JOIN $tbl_unidad_analizador ON $tbl_unidad.id_unidad = $tbl_unidad_analizador.id_unidad 
                INNER JOIN $tbl_analizador ON $tbl_analizador.id_analizador = $tbl_unidad_analizador.id_analizador 

                GROUP BY $tbl_unidad.id_unidad
                ORDER BY $tbl_analizador.nombre_analizador ASC, $tbl_unidad.nombre_unidad ASC";


			$qryArray = mysql_query($qry);
			while ($qryData = mysql_fetch_array($qryArray)) {
				$rv1[$x] = $qryData['id_conexion'];
				$rv2[$x] = $qryData['nombre_analizador'];
				$rv3[$x] = $qryData['id_unidad'];
				$rv4[$x] = $qryData['nombre_unidad'];
				$x++;
			}
