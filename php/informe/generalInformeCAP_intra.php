<?php
    ...
    class generalInformePAT_Intra extends FPDF {
        ...
        public $nom_reto;
// Grafica
        public $verde = [161, 232, 191];
        public $rojo = [255, 152, 140];
        public $amarillo = [244, 208, 63];
        public $verdeOscuro = [118, 193, 148];
        public $verdeOscuroPlus = [82, 161, 94];
        public $amarilloOscuro = [224, 183, 45];
        public $rojoOscuro = [245, 121, 121];
        public $rojoOscuroPlus = [232, 68, 68];
        public $blanco = [255,255,255];
        public $negro = [50,50,50];
        ...
        function SetTextColorsRow($array){
            $this->textColorsRow = $array;
        }

        function obtenerCriterioMK($id_respuesta_pat, $id_respuestas_correctas){
        ...
        function obtenerCriterioCYH($id_respuesta_pat, $id_respuestas_correctas){
            if(is_numeric(array_search($id_respuesta_pat,$id_respuestas_correctas))){
                return true;
            } else {
                return false;
        ...
        function obtenerCriterioPIP($id_respuesta_pat, $id_respuestas_correctas){
            if(is_numeric(array_search($id_respuesta_pat,$id_respuestas_correctas))){
                return true;
            } else {
                return false;
        ...
        function obtenerCriterioNGC($valor){
            $valor = intval($valor);
        ...
        function obtenerCriterioHER2($id_respuesta_pat, $id_respuestas_correctas){
            if(is_numeric(array_search($id_respuesta_pat,$id_respuestas_correctas))){
                return true;
            } else {
                return false;
        ...
        function obtenerCriterioPAP($valor){
        ...
        function obtenerCriterioPM2($nom_respuesta_pat, $nom_respuesta_correcta){
            /**
             * 0 = Generar si es rojo
             * 1 = Verde
             * 2 = N/A y no imprimir color
             */
            if($nom_respuesta_correcta == "N/A" || $nom_respuesta_correcta == "No aplica"){
                return 2;
            } else if($this->GetTypeDistractorPM2($nom_respuesta_pat) == $this->GetTypeDistractorPM2($nom_respuesta_correcta)){
                return 1;
            } else {
                return 0;
            }
        ...
        function GetTypeDistractorPM2($nom_distractor = ""){
            if($nom_distractor == "Grado 0" || $nom_distractor == "< 1%" || $nom_distractor == "Sin tinción" || $nom_distractor == "ST" || $nom_distractor == "Negativo"){ // Si es negativo
                return "Negativo";
            } else if($nom_distractor == "Grado 1+" || $nom_distractor == "Grado 2+" || $nom_distractor == "Grado 3+" || $nom_distractor == "1% - 10%" || $nom_distractor == "11% - 50%" || $nom_distractor == "> 50%"){ // Es positivo
                return "Positivo";
        ...
        function generarGraficoMK($concordantes = 0, $noconcordantes = 0){
        ...
        function generarGraficoCYH($concordantes = 0, $noconcordantes = 0){
        ...
        function generarGraficoPIP($concordantes = 0, $noconcordantes = 0){
        ...
        function generarGraficoNGC($concordantes = 0, $discordanciamenor=0, $noconcordantes = 0){
            $fuente = "../pchart/fonts/calibri.ttf";
            $fuenteBold = "../pchart/fonts/Calibri_Bold.ttf";
        ...
        function generarGraficoPAP($concordantes = 0, $discordanciamenor=0, $noconcordantes = 0){
        ...
        function generarGraficoPM2($concordantes = 0, $noconcordantes = 0){
        ...
        function generarGraficoHER2($concordantes = 0, $noconcordantes = 0){
        ...
        function RowMK($data){
            ...
            for($i=0;$i<count($data);$i++)
            {
                ...
                $textColorArray=isset($this->textColorsRow[$i]) ? $this->textColorsRow[$i] : [40,40,40];
                $textR = $textColorArray[0];
                $textG = $textColorArray[1];
                $textB = $textColorArray[2];
        ...
        function Header(){
            if($this->CurOrientation == "P"){
                ...
                $this->Cell(206.5,270,"",1,0,'L',0);
                
                // Recuadro ancho completo
                $this->SetXY(8,8);
                $this->Cell(200.5,17,"",1,0,'L',0);
                ...
                // Logo Quik
                $this->Image('../../css/qlogo.png',12,12,15);
                
                // Linea divisora vertical
                $this->SetLineWidth(0.1);
                $this->SetDrawColor(174, 182, 191);
                $this->line(31,13,31,20);
                
                // Logo de QAP-PAT
                $this->Image('../../css/CAP.png',35,13.5,31);
                
                // Recuadro de convenciones del formato
                $this->SetXY(65,8);
                $this->SetDrawColor(28, 80, 164);
                $this->Cell(95,17,"","R",0,'L',0);
                ...
                $this->MultiCell(92,(17/4),"\nCOMPARACIÓN INTRALABORATORIO\n".$this->sigla_programa . " ". $this->envio_pat . " | " . $this->nom_reto,0,'C',0);
                
                // Seccion de los valores fijos del formato
                $this->SetFont('Arial','B',7);
                $this->SetXY(160,8);
                $this->Cell(24.2,4.25,"Código:","R",0,'C',0);
                $this->Cell(24.2,4.25,"ADI-FOR-57",0,1,'C',0);
                
                $this->SetXY(160,12.25);
                $this->Cell(24.2,4.25,"Vigente desde:","RT",0,'C',0);
                $this->Cell(24.2,4.25,"23-Sep-2020","T",1,'C',0);
    
                $this->SetXY(160,16.5);
                $this->Cell(24.2,4.25,"Versión:","RT",0,'C',0);
                $this->Cell(24.2,4.25,"1","T",1,'C',0);
    
                $this->SetXY(160,21);
                $this->Cell(24.2,4.25,"Página:","RT",0,'C',0);
                $this->Cell(24.2,4.25,"     " .$this->PageNo() . " de {nb}","T",1,'C',0);
                ...
                // Logo de QAP-PAT
                $this->Image('../../css/CAP.png',43,12.5,40);
                ...
                $this->Cell(32,4.25,"23-Sep-2020","T",1,'C',0);
    
                $this->SetXY(208,16.5);
                $this->Cell(32,4.25,"Versión:","RT",0,'C',0);
                $this->Cell(32,4.25,"1","T",1,'C',0);
